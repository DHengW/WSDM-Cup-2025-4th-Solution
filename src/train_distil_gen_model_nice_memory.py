...
class qWenSFTDataset(Dataset):
    ...
    def __init__(self, dataset, tokenizer, max_prompt_len, max_completion_len) -> None:
        ...
        # 收集结果
        for input_ids,attention_masks, lengths,labels, grads_a1, grads_b1 ,grads_a2 ,grads_b2 ,grads_a3 ,grads_b3 in results:
            self.input_ids_list.append(input_ids)
            self.attention_masks_list.append(attention_masks)
            self.input_labels_list.append(labels)
            self.lengths.append(lengths)
            self.grads.append([grads_a1, grads_b1 ,grads_a2 ,grads_b2 ,grads_a3 ,grads_b3])
    ...
    def _process_single_entry(self, data_entry):
        ...
        labels = data['label']
        grads_a1 = data['winner_model_a1']
        grads_a2 = data['winner_model_a2']
        grads_a3 = data['winner_model_a3']
        grads_b1 = data['winner_model_b1']
        grads_b2 = data['winner_model_b2']
        grads_b3 = data['winner_model_b3']
        return features['input_ids'],features['attention_mask'],features['length'], labels, grads_a1, grads_b1 ,grads_a2 ,grads_b2 ,grads_a3 ,grads_b3
    ...
    def collate_fn(self, instances):
        ...
        grads = [feature["grads"] for feature in instances]
...
### 是否pad？vllm不做pad好像
        ...
        batch["grads"] = torch.stack(grads)

        return batch
        # features["labels"] = torch.tensor(labels, dtype=torch.long)
        # features["grads"] = torch.stack(grads)
        # return features
...
def main():
    ...
    if args.global_rank <=0:
        writer = SummaryWriter(f'./ds_tensorboard_logs/{proj_name}_fold{fold}_step1_model_tensorboard')
    ...
    with open(args.train_dataset_path, 'rb') as f:
        train_data = pickle.load(f)#.sample(n=200)

    if args.debug_code:
        train_data = train_data.sample(n=min(train_data.shape[0], 2000))
    with open(args.dev_dataset_path, 'rb') as f:
        eval_data = pickle.load(f)#.sample(n=100)        

    distil_data_path1 = os.path.join(args.distil_path1,f"fold{fold}/{fold}_preds.parquet")
    distil_data_path2 = os.path.join(args.distil_path2,f"fold{fold}/{fold}_preds.parquet")
    distil_data_path3 = os.path.join(args.distil_path3,f"fold{fold}/{fold}_preds.parquet")
    # distil_data_path4 = os.path.join(args.distil_path4,f"fold{fold}/{fold}_preds.parquet")
    with open(distil_data_path1,'rb') as f:
        distil_data1 = pickle.load(f)
    with open(distil_data_path2,'rb') as f:
        distil_data2 = pickle.load(f)
    with open(distil_data_path3,'rb') as f:
        distil_data3 = pickle.load(f)
    # with open(distil_data_path4,'rb') as f:
    #     distil_data4 = pickle.load(f)

    distil_data1 = distil_data1.rename(columns={'winner_model_a':'winner_model_a1','winner_model_b':'winner_model_b1'})
    distil_data2 = distil_data2.rename(columns={'winner_model_a':'winner_model_a2','winner_model_b':'winner_model_b2'})
    distil_data3 = distil_data3.rename(columns={'winner_model_a':'winner_model_a3','winner_model_b':'winner_model_b3'})
    # distil_data4 = distil_data4.rename(columns={'winner_model_a':'winner_model_a4','winner_model_b':'winner_model_b4'})
    print_rank_0(f"before merge shape: {train_data.shape},{eval_data.shape}", args.global_rank)
    train_data = train_data.drop(columns=['winner_model_a','winner_model_b']).merge(
        distil_data1[['id','response_a','response_b','winner_model_a1','winner_model_b1']],
        on=['id','response_a','response_b'],
        how='inner').merge(distil_data2[['id','response_a','response_b','winner_model_a2','winner_model_b2']],
                           on=['id','response_a','response_b'],
                           how='inner').merge(distil_data3[['id','response_a','response_b','winner_model_a3','winner_model_b3']],
                               on=['id','response_a','response_b'],
                               how='inner')
    eval_data = eval_data.merge(distil_data1[['id','response_a','response_b','winner_model_a1','winner_model_b1']],
                                  on=['id','response_a','response_b'],
                                 how='inner').merge(distil_data2[['id','response_a','response_b','winner_model_a2','winner_model_b2']],
                                  on=['id','response_a','response_b'],
                                 how='inner').merge(distil_data3[['id','response_a','response_b','winner_model_a3','winner_model_b3']],
                                  on=['id','response_a','response_b'],
                                 how='inner')
    print_rank_0(f"after merge shape: {train_data.shape},{eval_data.shape}", args.global_rank)
    if args.debug_code:
    ...
    val_begin = int(len(train_dataloader)*0.67)
    print(f'Running fold {fold}!!——————>Val start at {val_begin}')
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            global_step += 1
            batch = to_device(batch, device)
            outputs = model(**batch, use_cache=False, use_distillation=True)##test
            loss = outputs.loss
            model.backward(loss)
            model.step()
            ...
            if (global_step + 1) % args.gradient_accumulation_steps == 0:
                total_loss = 0.
            if (args.save_batch_steps and (global_step + 1) % args.save_batch_steps == 0 and global_step > val_begin) or ((global_step+1) % len(train_dataloader) == 0) or (global_step % len(train_dataloader) == 0):### continue
                ...
                if acc >= best_val_acc:
                    print_rank_0(
                        ...
                        args.global_rank)
                    if args.global_rank == 0:
                        model.save(os.path.join(args.output_dir, f"fold{fold}/kl_distil4_encode/{epoch}best_val_acc_model"))
                        print('saved success!!')
                    best_val_acc = acc
                    no_improve_epoch = 0
                else:
        ...
        if epoch == args.num_train_epochs - 1:
            model.save(os.path.join(args.output_dir, f"fold{fold}/kl_distil4_encode/epoch_{epoch}_model"))
        #     save_model(args, model, tokenizer, f"epoch_{epoch}_model")
...
def parse_args():
    ...
    parser.add_argument('--tensorboard_path',
                        type=str,
                        default=".")
    parser.add_argument('--distil_path1',
                        type=str)
    parser.add_argument('--distil_path2',
                        type=str)
    parser.add_argument('--distil_path3',
                        type=str)
