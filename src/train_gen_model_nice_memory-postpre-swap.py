...
from model import EediRanker,WSDMRanker
...
class qWenSFTDatasetVal(Dataset):
    ...
    def __init__(self, dataset, tokenizer, max_prompt_len, max_completion_len) -> None:
        ...
        # 收集结果
        for input_ids,attention_masks, lengths,labels,input_ids2,attention_masks2,labels2 in results:
            self.input_ids_list.append(input_ids)
            self.attention_masks_list.append(attention_masks)
            self.input_labels_list.append(labels)
            self.input_ids_lists.append(input_ids2)
            self.attention_masks_lists.append(attention_masks2)
            self.input_labels_lists.append(labels2)
    ...
    def __getitem__(self, idx):
        return [dict(input_ids=torch.tensor(self.input_ids_list[idx],
                ...
                dict(input_ids=torch.tensor(self.input_ids_lists[idx],
                                           dtype=torch.long),
                    attention_mask=torch.tensor(self.attention_masks_lists[idx],
                                           dtype=torch.long),
                    labels=torch.tensor(self.input_labels_lists[idx],
                                        dtype=torch.long),
                   lengths = self.lengths[idx])
               ]
    ...
    def collate_fn(self, original_list):
        instances = [item for sublist in original_list for item in sublist]
        features = [
            {
...
def main():
    ...
    if args.global_rank <=0:
        writer = SummaryWriter(f'./tf-logs/ds_tensorboard_logs/{proj_name}_step1_model_tensorboard')
    ds_config = get_train_ds_config(offload=args.offload,
                                    stage=args.zero_stage,
                                    enable_tensorboard=True,
                                    tb_path=args.tensorboard_path,
                                    tb_name=f"{proj_name}_step1_model")
    ...
    model = WSDMRanker(base_model, tokenizer)
    ...
    eval_dataset = qWenSFTDatasetVal(eval_data, tokenizer, args.max_prompt_len, args.max_completion_len)
    ...
    train_dataloader = DataLoader(train_dataset,
                                  ...
                                  batch_size=int(args.per_device_train_batch_size/2),
                                  pin_memory=True)
    eval_dataloader = DataLoader(eval_dataset,
                                 shuffle=(eval_sampler is None),
                                 collate_fn=data_collator,
                                 sampler=eval_sampler,
                                 batch_size=int(args.per_device_eval_batch_size/2),
                                 pin_memory=True)
    ...
    def evaluation_dist(model, eval_dataloader):
        ...
        acc2 = (labels==preds).sum()/preds.shape[0]
        print("Acc score:", acc,"ACC:",acc2 , "number:", len(predicts))
        return acc
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (global_step + 1) % args.gradient_accumulation_steps == 0:
                total_loss = 0.
            if (args.save_batch_steps and (global_step + 1) % args.save_batch_steps == 0 and global_step > 1300) or (global_step % len(train_dataloader) == 0) or ((global_step+1) % len(train_dataloader) == 0):
                ...
                if acc >= best_val_acc:
                    print_rank_0(
                        ...
                        args.global_rank)
                    if args.global_rank == 0:
                        model.save(os.path.join(args.output_dir, f"best_val_acc_model"))
                        print('saved success!!')
        ...
        if args.save_per_epoch == 1:
            if args.global_rank == 0:
                model.save(os.path.join(args.output_dir, f"best_val_acc_model"))
                print('saved each success!!')
        # 保存最后一轮
        if epoch == args.num_train_epochs - 1:
            if args.global_rank == 0:
                model.save(os.path.join(args.output_dir, f"best_val_acc_model"))
                print('saved last success!!')
