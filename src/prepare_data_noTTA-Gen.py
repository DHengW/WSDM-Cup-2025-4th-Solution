...
def process_row_val(row):
    ...
    if query_len + answer_a_len + answer_b_len > MAX_LENGTH_v:
        query = query if len(tokenizer.encode(query)) < prompt_len else tokenizer.decode(
            tokenizer.encode(query)[:prompt_len])
        query_len = len(tokenizer.encode(query))
        if query_len + answer_a_len + answer_b_len > MAX_LENGTH_v:
            remain_len = MAX_LENGTH_v - query_len
            token_answer_a = tokenizer.encode(answer_a)
            token_answer_b = tokenizer.encode(answer_b)
            if len(token_answer_a) > len(token_answer_b):
                while len(token_answer_a) + len(token_answer_b) > remain_len and len(token_answer_a) > len(
...
with Pool(cpu_count()) as pool:
    ...
    texts_train = list(tqdm(pool.imap(process_row_train, train_dicts), total=len(train)))

with Pool(cpu_count()) as pool:
    # 使用imap保持结果的顺序，并且提供进度条
    texts_val = list(tqdm(pool.imap(process_row_val, train_dicts), total=len(train)))
# 将处理后的文本添加到DataFrame中
train['text'] = texts_train[:]
train['text_val'] = texts_val[:]
...
# train_dicts = train_reverse.to_dict(orient='records')
# with Pool(cpu_count()) as pool:
#     # 使用imap保持结果的顺序，并且提供进度条
#     texts_train = list(tqdm(pool.imap(process_row_train, train_dicts), total=len(train_reverse)))
# # 将处理后的文本添加到DataFrame中
# train_reverse['text'] = texts_train[:]
# train_reverse['reverse'] = True
#####reverse
train['reverse'] = False
# train = pd.concat([train, train_reverse], axis=0)##reverse
train['order_index'] = list(range(len(train)))

######################### load 33k data and do the same thing like train ###############################
train2 = pd.read_csv('../data/notie_lmsys.csv')
def do(row):
...
with Pool(cpu_count()) as pool:
    # 使用imap保持结果的顺序，并且提供进度条
    texts = list(tqdm(pool.imap(process_row_train, train_dicts2), total=len(train2)))
...
# train_dicts2 = train_reverse2.to_dict(orient='records')
# with Pool(cpu_count()) as pool:
#     # 使用imap保持结果的顺序，并且提供进度条
#     texts = list(tqdm(pool.imap(process_row_train, train_dicts2), total=len(train_reverse2)))

# train_reverse2['text'] = texts[:]
# train_reverse2['reverse'] = True
#####reverse
train2['reverse'] = False
# train2 = pd.concat([train2, train_reverse2], axis=0)#####reverse
...
for fold in range(5):
    tra = train[train['fold'] != fold].drop(columns=['text_val'])
    dev = train[train['fold'] == fold]
    dev = dev[dev['reverse']==False].drop(columns=['text']).rename(columns={'text_val':'text'})
    tra = pd.concat([tra, train2], axis=0)
    print(fold, tra.shape, dev.shape)

    with open(f"../data/processed_data/{save_name}fold{fold}/dev_10000.parquet", 'wb') as f:
        pickle.dump(dev, f)
    with open(f"../data/processed_data/{save_name}fold{fold}/train_2048.parquet", 'wb') as f:
        pickle.dump(tra, f)
