...
def main():
    ...
    if args.global_rank <=0:
        writer = SummaryWriter('./step1_model_tensorboard')#/root/tf-logs/ds_tensorboard_logs
    ...
    with open(args.train_dataset_path, 'rb') as f:
        train_data = pickle.load(f)#.sample(n=2000)
    ...
    def evaluation_dist(model, eval_dataloader):
        ...
        print("Acc score:", acc,"ACC:",acc2 ,"loss score:", loss, "number:", len(predicts))
        return acc
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (global_step + 1) % args.gradient_accumulation_steps == 0:
                total_loss = 0.
            if (args.save_batch_steps and (global_step + 1) % args.save_batch_steps == 0 and global_step > 1300) or (global_step % len(train_dataloader) == 0):
                ...
                if acc >= best_val_acc:
                    print_rank_0(
                        ...
                        args.global_rank)
                    if args.global_rank == 0:
                        model.save(os.path.join(args.output_dir, f"pretrain_best_val_acc_model"))
                        print('saved success!!')
